...
void GLTexture::LoadBMP(char *name)
{
	...
	wchar_t wname[256];
	mbstowcs(wname, name, 256); // ÊÍæíá ãä char* Åáì wchar_t*
	// Load the bitmap and assign our pointer to it
	TextureImage[0] = auxDIBImageLoad(wname);
...
void GLTexture::LoadBMPResource(char *name)
{
	...
	HRSRC hrsrc = FindResource(0, wname, RT_BITMAP);
...
void GLTexture::LoadTGAResource(char *name)
{
	...
	HRSRC hrsrc = FindResource(0, wname, L"TGA");
