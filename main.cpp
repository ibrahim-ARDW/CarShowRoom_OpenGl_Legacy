...
int DrawGLScene(GLvoid)                  // Here's Where We Do All The Drawing
{
    ...
    if (camera.jumping)
    {
        camera.jumpTime += 0.0005f; // ÃÈØÃ: Þáøá ÞíãÉ ÇáÒíÇÏÉ (0.01f ÈÏá 0.02f)

        float jumpHeight = 4.0f; // ÇÑÊÝÇÚ ÃÚáì ááÞÝÒ (ãËáÇð 2.5 ãÊÑ)

        // ãÚÇÏáÉ ÇáÞÝÒ: sin(pi * t) * jumpHeight
        camera.y = camera.jumpStartY + sinf(camera.jumpTime * 3.14159f) * jumpHeight;

        if (camera.jumpTime >= 1.0f)
        {
            camera.jumping = false;
            camera.y = camera.height; // ÇáÚæÏÉ áÇÑÊÝÇÚ ÇáãÔí ÇáØÈíÚí
        }
    }
