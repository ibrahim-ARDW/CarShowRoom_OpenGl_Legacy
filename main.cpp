...
int InitGL(GLvoid)                    // All Setup For OpenGL Goes Here
{
    ...
    if (!carModel.Load("carOBJ/car.obj")) {
        MessageBox(NULL, L"áã íÊã ÇáÚËæÑ Úáì ãáÝ ÇáÓíÇÑÉ! ÊÃßÏ ãä æÌæÏå ÈÌÇäÈ ãáÝ ÇáÜ exe", L"ÎØÃ Ýí ÇáÊÍãíá", MB_OK);
        return FALSE; // ÝÔá ÇáÊÍãíá
    }
...
int DrawGLScene(GLvoid)                  // Here's Where We Do All The Drawing
{
    ...
    // --- 1. ÑÓã ÇáÚÔÈ (ÇáÃÑÖíÉ) ---
    glEnable(GL_TEXTURE_2D);    // ÊÝÚíá ÇáÊßÓÊÔÑ
    glDisable(GL_LIGHTING);     // ÊÚØíá ÇáÅÖÇÁÉ ãÄÞÊÇð áíßæä ÇáÚÔÈ ÓÇØÚÇð

    // ÇáÍá ÇáÓÍÑí: ÅÚÇÏÉ Çááæä ááÃÈíÖ ÇáÕÇÝí 100% 
    ...
    glDisable(GL_LIGHTING);     // ÊÝÚíá ÇáÅÖÇÁÉ áÅÙåÇÑ ãÌÓã ÇáÓíÇÑÉ
   // glEnable(GL_LIGHT0);        // ÊÝÚíá ÇáÖæÁ ÇáÃæá

    glPushMatrix();
    glTranslated(0, -1, 0); // ÑÝÚ ÇáÓíÇÑÉ ÞáíáÇð Úä ÇáÃÑÖ
    glScalef(0.7f, 0.7f, 0.7f);
    carModel.Draw();         // ÑÓã ÇáãæÏíá ÇáÓÑíÚ (Display List)
    glPopMatrix();

    glDisable(GL_LIGHTING);     // ÊÚØíá ÇáÅÖÇÁÉ áÈÇÞí ÇáÚäÇÕÑ (ÇÎÊíÇÑí)
